...
function validateOpsProfile(op) {
  if (!op || typeof op !== "object") return "opsProfile is required";

  if (!op.fulfillmentModel || typeof op.fulfillmentModel !== "object") return "opsProfile.fulfillmentModel is required";
  if (!op.operationalPressures || typeof op.operationalPressures !== "object") return "opsProfile.operationalPressures is required";

  const pressures = op.operationalPressures;
  const keys = ["service", "labor", "inventory", "returns", "carrier"];
  for (const k of keys) {
    if (!isNumberInRange(pressures[k], 1, 5)) return `opsProfile.operationalPressures.${k} must be 1..5`;
  }

  if (!Array.isArray(op.likelyWmsGaps) || op.likelyWmsGaps.length < 3 || !op.likelyWmsGaps.every((x) => isNonEmptyString(x))) {
    return "opsProfile.likelyWmsGaps must be a string[] (len >= 3)";
  }

  if (!Array.isArray(op.diagnoses) || op.diagnoses.length < 2) return "opsProfile.diagnoses must be an array (len >= 2)";
  for (const d of op.diagnoses) {
    if (!d || typeof d !== "object") return "opsProfile.diagnoses entry must be an object";
    if (!isNonEmptyString(d.assertion)) return "opsProfile.diagnoses.assertion is required";
    if (!isStringArray(d.evidenceNeeded) || d.evidenceNeeded.length < 2) return "opsProfile.diagnoses.evidenceNeeded must be string[] (len >= 2)";
    if (!isNumberInRange(d.confidence, 0, 1)) return "opsProfile.diagnoses.confidence must be 0..1";
  }
...
function validateCombined(out) {
  ...
  if (!isNumberInRange(c.confidence, 0, 1)) return "classification.confidence must be 0..1";
  if (!isStringArray(c.rationale) || c.rationale.length < 2) return "classification.rationale must be a string[]";

  const opsErr = validateOpsProfile(out.opsProfile);
  if (opsErr) return opsErr;
...
function buildPrimaryPrompt({ companyName, notes }) {
  ...
  return (
    `Create a combined research brief + warehouse ops profile + dashboard classification for \"${companyName}\".\n` +
    `Return ONLY valid JSON (no markdown, no commentary, no code fences).\n` +
    `The response MUST be a single JSON object with exactly these keys: researchText, classification, opsProfile.\n\n` +
    ...
    `opsProfile MUST be exactly this JSON shape (Mode 2: executive-direct hard diagnosis):\n` +
    `{\n` +
    `  \"fulfillmentModel\": {\n` +
    `    \"channels\": {\"b2b\": boolean, \"b2c\": boolean, \"ecom\": boolean, \"retailReplen\": boolean},\n` +
    `    \"eachCasePalletMix\": \"each\"|\"case\"|\"pallet\"|\"mixed\",\n` +
    `    \"cutoffsPresent\": boolean\n` +
    `  },\n` +
    `  \"operationalPressures\": {\n` +
    `    \"service\": 1-5, \"labor\": 1-5, \"inventory\": 1-5, \"returns\": 1-5, \"carrier\": 1-5\n` +
    `  },\n` +
    `  \"likelyWmsGaps\": string[],\n` +
    `  \"diagnoses\": [\n` +
    `    {\"assertion\": string, \"evidenceNeeded\": string[], \"confidence\": 0..1}\n` +
    `  ]\n` +
    `}\n\n` +
    `Rules for researchText:\n- Use '-' for bullets only.\n- Keep bullets short (<= 1 line).\n- No long paragraphs.\n\n` +
    `Rules for classification:\n- confidence must be 0 to 1\n- rationale must be 3-6 short strings\n\n` +
    `Rules for opsProfile:\n- Keep diagnoses executive-direct and warehouse-specific.\n- Do NOT mention competitors or specific WMS vendors.\n- EvidenceNeeded should be WMS-measurable signals (scan compliance, task travel time, dwell time, adjustments, aged exceptions).\n\n` +
    `Company: ${companyName}\n` +
    notesBlock
  );
}
...
function buildRepairPrompt({ badJsonText }) {
  return (
    ...
    `Required keys: researchText, classification, opsProfile.\n` +
    `classification keys: industry, subVertical, confidence, rationale.\n` +
    `opsProfile keys: fulfillmentModel, operationalPressures, likelyWmsGaps, diagnoses.\n\n` +
...
async function getCombinedWithRetry({ apiKey, model, system, companyName, notes }) {
  ...
  const raw1 = await anthropicMessages({ apiKey, model, system, user: user1, maxTokens: 1400, timeoutMs: 25000 });
  ...
  try {
    ...
    const raw2 = await anthropicMessages({ apiKey, model, system, user: user2, maxTokens: 1200, timeoutMs: 25000 });
...
exports.handler = async (event) => {
  ...
  try {
    ...
    return json(200, {
      researchText: resp.out.researchText,
      classification: resp.out.classification,
      opsProfile: resp.out.opsProfile,
