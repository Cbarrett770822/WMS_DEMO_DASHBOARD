...
import {
  Bar, BarChart as RBarChart, CartesianGrid, Cell, ComposedChart as RComposedChart,
  Legend, Line, LineChart as RLineChart, Pie, PieChart as RPieChart,
...
function DashRoute() {
      ...
      <ShipmentsOtifCombo rows={monthly} />
          ...
          {kpis.length ? (
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {kpis.map((w) => (
                <WidgetCard key={w.id} widget={w} monthly={monthly} tables={tables} />
              ))}
  ...
  return (
    <div key={w.id} className={wide ? "md:col-span-2" : ""}>
      <WidgetCard widget={w} monthly={monthly} tables={tables} />
    </div>
  );
})} 
            </div>
  ...
  <div className="grid gap-4">
    <div className="grid gap-4 md:grid-cols-2">
      <Panel title="Exceptions Mix" description="Open exceptions by type">
        <DonutChart rows={(tables as any).exceptionsSummary || []} nameKey="exceptionType" valueKey="open" />
      </Panel>

      <Panel title="Carrier Mix" description="Shipments by carrier">
        <DonutChart rows={(tables as any).carrierPerformance || []} nameKey="carrier" valueKey="shipments" />
      </Panel>

      <div className="md:col-span-2">
        <Panel title="Exceptions Aging (Stacked)" description="Open vs New Today">
          <StackedExceptionsAging rows={(tables as any).exceptionsSummary || []} />
        </Panel>
      </div>

      <Panel title="Backlog Aging" description="Orders by aging bucket">
        <BacklogAgingBars rows={(tables as any).backlogAging || []} />
      </Panel>

      <Panel title="Delay Distribution (Histogram)" description="Weighted by late orders">
        <DelayHistogram rows={(tables as any).slaByCustomer || []} />
      </Panel>
    </div>

    <Panel title="Action Plans" description="Heuristic next best actions">
      {(() => {
        const ex = ((tables as any).exceptionsSummary || []) as any[];
        const backlog = ((tables as any).backlogAging || []) as any[];
        const sla = ((tables as any).slaByCustomer || []) as any[];
